...
Do not use a Deno [import map](https://deno.land/manual/basics/import_maps#import-maps). For now Dropserver uses import maps internally, and does not currently attempt to merge any other import maps.

#### 🚫 Deno.json

Do not use a [`deno.json`file](https://docs.deno.com/runtime/manual/getting_started/configuration_file).
