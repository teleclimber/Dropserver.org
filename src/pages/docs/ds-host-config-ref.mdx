...
`sandbox.use-bubblewrap`: Causes `ds-host` to wrap its calls to Deno in a [Bubblewrap](https://github.com/containers/bubblewrap) sandbox. The `bwrap`` executable must be installed on the system and accessible to the Dropserver user. See [here for full details](/docs/bubblewrap).
