...
- On a Linux host, acquire the [latest version of ds-host](https://github.com/teleclimber/Dropserver/releases/) and [Deno](https://deno.land). Optionally install Bubblewrap "bwrap" from your package manager.
- Point a domain and its subdomains to your host.
...
<Note>On more recent versions of Ubuntu, [apparmor conflicts with Bubblewrap](https://github.com/teleclimber/Dropserver/issues/139). See the links in the issue for workarounds, or turn off bubblewrap in the config, or use an older version of Ubuntu.</Note>
